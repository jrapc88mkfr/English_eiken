    ...
    with open(filename, encoding="utf-8") as f:
        ...
        for line_no, row in enumerate(reader, start=1):
            ...
            if n == 7:
                ...
                if question and answer and japanese:
                    words.append(WordQuiz(question, [c1, c2, c3, c4], answer, japanese))
                else:
...
def load_missed_words_unique(player):
    """セーブデータから読み込み、Word系オブジェクトのリストにして返す。"""
    raw = storage_get(f"missed_words.{_safe_player_key(player)}", default=[])
    seen = {}
    for item in raw:
        if isinstance(item, list) and len(item) >= 2:
            a, b = item[0], item[1]
            seen[(a, b)] = WordBasic(a, b)
    return list(seen.values())


def save_missed_word(key_a, key_b, player):
    key = f"missed_words.{_safe_player_key(player)}"
    raw = storage_get(key, default=[])
    raw.append([key_a, key_b])
    storage_set(key, raw)


def remove_missed_word(player, key_a, key_b):
    """復習をマスターした単語を「間違えた単語」リストから完全に削除する。"""
    key = f"missed_words.{_safe_player_key(player)}"
    raw = storage_get(key, default=[])
    raw = [item for item in raw
           if not (isinstance(item, list) and len(item) >= 2
                   and item[0] == key_a and item[1] == key_b)]
    storage_set(key, raw)
...
class WordGame:
    ...
    def check_answer(self, choice):
        ...
        if is_correct:
            ...
            a, b = self.correct_word.save_key()
            save_missed_word(a, b, self.player)
