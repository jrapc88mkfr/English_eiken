...
# ------------------------------------------------------------------
FIREBASE_DB_URL = ""  # ← ここにご自身のURLを入れてください（末尾の / は不要）
...
# --- 間違えた単語もFirebaseで端末をまたいで共有する ---
_firebase_missed_result = {"data": None}


def _firebase_fetch_missed_sync(player):
    import urllib.request
    import urllib.parse
    url = f"{FIREBASE_DB_URL}/missed_words/{urllib.parse.quote(player, safe='')}.json"
    try:
        with urllib.request.urlopen(url, timeout=5) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            return data or []
    except Exception as e:
        print(f"[Firebase] 間違えた単語の取得に失敗しました: {e}")
        return None


def _firebase_push_missed_sync(player, records):
    import urllib.request
    import urllib.parse
    url = f"{FIREBASE_DB_URL}/missed_words/{urllib.parse.quote(player, safe='')}.json"
    try:
        req = urllib.request.Request(
            url, data=json.dumps(records).encode("utf-8"), method="PUT",
            headers={"Content-Type": "application/json"},
        )
        urllib.request.urlopen(req, timeout=5)
    except Exception as e:
        print(f"[Firebase] 間違えた単語の送信に失敗しました: {e}")


async def _firebase_fetch_missed_task(player):
    import urllib.parse
    try:
        resp = await _pyodide_http.pyfetch(
            f"{FIREBASE_DB_URL}/missed_words/{urllib.parse.quote(player, safe='')}.json")
        data = await resp.json()
        _firebase_missed_result["data"] = data or []
    except Exception as e:
        print(f"[Firebase] 間違えた単語の取得に失敗しました: {e}")
        _firebase_missed_result["data"] = []


async def _firebase_push_missed_task(player, records):
    import urllib.parse
    try:
        await _pyodide_http.pyfetch(
            f"{FIREBASE_DB_URL}/missed_words/{urllib.parse.quote(player, safe='')}.json",
...
def firebase_fetch_missed_start(player):
    if not firebase_enabled() or not player:
        return
    if HAS_PYODIDE_HTTP:
        asyncio.ensure_future(_firebase_fetch_missed_task(player))
    else:
        _firebase_missed_result["data"] = _firebase_fetch_missed_sync(player)


def firebase_push_missed(player, records):
    if not firebase_enabled() or not player:
        return
    if HAS_PYODIDE_HTTP:
        asyncio.ensure_future(_firebase_push_missed_task(player, records))
    else:
        _firebase_push_missed_sync(player, records)


def firebase_take_missed_result():
    data = _firebase_missed_result["data"]
    _firebase_missed_result["data"] = None
    return data
...
class WordGame:
    ...
    def merge_cloud_ranking_if_ready(self):
        """Firebaseからの取得結果が届いていたら、ローカルとクラウドを見比べて
        お互いに「高い方」に揃える（届いていなければ何もしない）。
        これにより、Firebase設定前にオフラインで達成したスコアなども、
        次にこの画面を開いた時に自動でクラウド側へ反映される。"""
        cloud = firebase_take_ranking_result()
        if cloud is None:
            return
        cloud = dict(cloud)

        # クラウドの方が高ければ、ローカルへ取り込む
        for player, score in cloud.items():
            ...
            if player not in self.cached_best_scores or score > self.cached_best_scores[player]:
                ...
                save_ranking_entry(player, score)

        # ローカルの方が高ければ（例：Firebase未設定の間にオフラインで達成した記録など）、
        # クラウドへ送り返して他の端末にも反映されるようにする
        for player, score in self.cached_best_scores.items():
            cloud_score = cloud.get(player)
            try:
                cloud_score = int(cloud_score) if cloud_score is not None else None
            except (TypeError, ValueError):
                cloud_score = None
            if cloud_score is None or score > cloud_score:
                firebase_push_ranking(player, score)

    def merge_cloud_missed_words_if_ready(self):
        """Firebaseから届いた「間違えた単語」を、ローカルの一覧に合体させる
        （どちらか一方にしか無いものも含め、両方の和集合にする）。"""
        cloud = firebase_take_missed_result()
        if cloud is None:
            return
        local_words = load_missed_words_unique(self.player)
        local_keys = {w.save_key() for w in local_words}
        merged = list(local_words)
        changed = False
        for item in cloud:
            w = word_from_missed_record(item)
            if w is not None and w.save_key() not in local_keys:
                merged.append(w)
                local_keys.add(w.save_key())
                changed = True
        if changed:
            storage_set(f"missed_words.{_safe_player_key(self.player)}",
                        [w.to_missed_record() for w in merged])
            self.refresh_missed_count()
        # クラウド側にも、合体した最新の一覧を送り返しておく
        # （次に他の端末が読んだ時、ここでの分もちゃんと入っているように）
        firebase_push_missed(self.player, [w.to_missed_record() for w in merged])

    def sync_missed_words_to_cloud(self):
        """今のローカルの「間違えた単語」一覧を、まるごとFirebaseに送る。
        単語を追加した時・復習で卒業させた時に呼ぶ。"""
        if not firebase_enabled() or not self.player:
            return
        words = load_missed_words_unique(self.player)
        firebase_push_missed(self.player, [w.to_missed_record() for w in words])
    ...
    def select_player(self, name):
        self.player = name
        firebase_fetch_missed_start(name)  # 結果はSTATE_STARTに来てからupdate()で反映
        self.go_to_start()
    ...
    def update(self):
        ...
        if self.state == STATE_START:
            self.merge_cloud_missed_words_if_ready()
            self.update_start()
            return
    ...
    def check_answer(self, choice):
        ...
        if is_correct:
            ...
            self.sync_missed_words_to_cloud()
